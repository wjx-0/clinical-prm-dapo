...
def _active_weights(config: dict[str, Any]) -> dict[str, float]:
    ...
    active_profile = os.environ.get("REWARD_PROFILE") or config.get("active_profile") or "prm_v1"
    configured = profiles.get(active_profile) if isinstance(profiles.get(active_profile), dict) else None
    weights = dict(configured or DEFAULT_PROFILES.get(active_profile) or DEFAULT_PROFILES["prm_v1"])
...
def compute_score(data_source, solution_str, ground_truth, extra_info=None):
    ...
    components = {
        "answer": score_answer(solution_str, gold),
        "format": score_format(solution_str),
        "length": score_length(solution_str),
        "prm": score_prm(
            solution_str,
            ground_truth=gold,
        ...
        "consistency": score_consistency(solution_str, extra_info=extra_info),
    }
